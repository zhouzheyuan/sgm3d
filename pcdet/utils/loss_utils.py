...
def compute_fg_mask(gt_boxes2d, shape, downsample_factor=1, device=torch.device("cpu")):
    ...
    for b in range(B):
        for n in range(N):
            ...
            fg_mask[b,  min(v1,v2):max(v1,v2), min(u1,u2):max(u1,u2)] = True
...
class compute_vertex(torch.autograd.Function):
    '''
...
class find_convex_hull(torch.autograd.Function):
...
class odiou_3D(nn.Module):
    ...
    def forward(ctx, gboxes, qboxes, weights):
        '''

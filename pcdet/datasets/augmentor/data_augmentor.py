...
class DataAugmentor(object):
    ...
    def random_flip(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_flip, config=config)
        ...
        for cur_axis in config['ALONG_AXIS_LIST']:
            assert cur_axis in ['horizontal']
            points, images, depth_maps, gt_boxes2d, gt_boxes = getattr(augmentor_utils, 'random_flip_%s' % cur_axis)(
                points, images, depth_maps, gt_boxes2d, gt_boxes, calib,
            )
    ...
    def noaug(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.noaug, config=config)
        return data_dict

...
def random_flip_horizontal(points, image, depth_map, gt_boxes2d, gt_boxes, calib):
    """
    Performs random horizontal flip augmentation
    Args:
        points: (M, 3 + C)
        ...
        calib: calibration.Calibration, Calibration object
    Returns:
        aug_points: (M, 3 + C), Augmented point
        aug_image: (H_image, W_image, 3), Augmented image
        aug_depth_map: (H_depth, W_depth), Augmented depth map
        aug_gt_boexes2d: (N, 4), Augmented 2D box labels in image coordinates [x1, y1, x2, y2]
        aug_gt_boxes: (N, 7), Augmented 3D box labels in LiDAR coordinates [x, y, z, w, l, h, ry]
    """
    # Randomly augment with 50% chance
    enable = np.random.choice([False, True], replace=False, p=[0.5, 0.5])
    ...
    if enable:
        #Flip points
        aug_points = copy.copy(points)
        aug_points[:, 1] = -aug_points[:, 1]
        ...
        aug_gt_boxes2d[:,0] = image.shape[1] - aug_gt_boxes2d[:,0]
        aug_gt_boxes2d[:,2] = image.shape[1] - aug_gt_boxes2d[:,2]
    ...
    return aug_points, aug_image, aug_depth_map, aug_gt_boxes2d, aug_gt_boxes